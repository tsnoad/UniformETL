...
Class MemberEpdpStatuses {
	...
	function get_dst_members_ecpd_statuses($chunk_id) {
		$dst_member_ecpd_statuses_query = runq("SELECT DISTINCT e.member_id, e.participant, e.coordinator FROM epdp_statuses e INNER JOIN chunk_member_ids ch ON (ch.member_id=e.member_id) WHERE ch.chunk_id='{$chunk_id}';");
	...
	function hook_api_get_member($data) {
		return array("epdp.participant as epdp_particpant, epdp.coordinator as epdp_coordinator", "LEFT JOIN epdp_statuses epdp ON (epdp.member_id=m.member_id)");
	}
}
