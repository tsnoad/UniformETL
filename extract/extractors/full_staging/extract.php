...
class ExtractFullStaging {
	...
	function start($source_path, $source_timestamp, $source_md5) {
		...
		$get_columns = New ExtractFullStagingGetColumns;
		...
		passthru("tar -czf ".Conf::$extractor_config['full_staging']['output_path']."/dump_{$this->extract_id}.tgz -C {$this->extractdir} dump.sql tabout".implode(".sql tabout", $models->sources).".sql");
...
*/
	...
	function get_extract_id($source_path, $source_timestamp, $source_md5) {
		try {
			...
			runq("INSERT INTO extract_processes (extract_id, extractor, extract_pid, models) VALUES ('".db_escape($this->extract_id)."', 'full_staging', '".db_escape(getmypid())."', '".db_escape(json_encode(Conf::$do_transforms))."');");
		
			//record information about the source file
			runq("INSERT INTO extract_full_staging (extract_id, source_path, source_timestamp, source_md5) VALUES ('".db_escape($this->extract_id)."', '".db_escape($source_path)."', '".db_escape($source_timestamp)."', '".db_escape($source_md5)."');");
	...
	function run_convert($sources) {
		...
		foreach ($sources as $i => $source) {
			...
			passthru("/etc/uniformetl/extract/extractors/full_staging/aaaaarf.sh {$this->extractuntardir}/tabout{$source}.dat {$this->extractdir}/tabout{$source}.sql");
		}
	}
	...
	function create_copy_sql($table_columns, $sources, $tables) {
		...
		foreach ($sources as $i => $source) {
			...
			$sql .= db_choose(
				db_pgsql("COPY {$table} (".implode(", ", $table_columns[$source]).") FROM '%{extract_dir}/tabout{$source}.sql' DELIMITER '|' NULL AS '' CSV QUOTE AS $$'$$ ESCAPE AS ".'$$\$$'.";\n\n"),
				db_mysql("LOAD DATA LOCAL INFILE '%{extract_dir}/tabout{$source}.sql' INTO TABLE {$table} COLUMNS TERMINATED BY '|' ENCLOSED BY '\'';\n\n")
			);
		}
...
function execute_sed($extractuntardir, $extractdir, $sources) {
	foreach ($sources as $i => $source) {
		...
		shell_exec("/etc/uniformetl/extract/extractors/full_staging/aaaaarf.sh {$extractuntardir}/tabout{$source}.dat {$extractdir}/tabout{$source}.sql");
	}
}
