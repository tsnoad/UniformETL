...
class ExtractFullStaged {
	...
	function get_extract_id($source_path, $source_timestamp, $source_md5) {
		try {
			...
			runq("INSERT INTO extract_processes (extract_id, extractor, extract_pid, models) VALUES ('".db_escape($this->extract_id)."', 'full_staged', '".db_escape(getmypid())."', '".db_escape(json_encode(Conf::$do_transforms))."');");
		
			//record information about the source file
			runq("INSERT INTO extract_full_staged (extract_id, source_path, source_timestamp, source_md5) VALUES ('".db_escape($this->extract_id)."', '".db_escape($source_path)."', '".db_escape($source_timestamp)."', '".db_escape($source_md5)."');");
	...
	function run_scp($source_path) {
		passthru("scp -i ".Conf::$extractor_config['full_staged']['identity']." ".Conf::$extractor_config['full_staged']['server'].":{$source_path} {$this->extractuntardir} > /etc/uniformetl/logs/extractlog");
	}
	...
	function run_tar($sources) {
		passthru("tar -xvzf {$this->extractuntardir}/*.tgz -C {$this->extractdir} dump.sql tabout".implode(".sql tabout", $sources).".sql");
	}
}
