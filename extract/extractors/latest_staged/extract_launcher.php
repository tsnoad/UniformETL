...
class ExtractLatestStagedLauncher {
	function start() {
		//helpful log message
	...
	function list_remote_dumps() {
		...
		$remote_command .= 'for file in '.escapeshellarg(Conf::$extractor_config['latest_staged']['dumps_path']).' ; do echo $file; stat --format=%Y $file; md5sum $file | cut -d " " -f 1; done';

		//run the command on the remote server
		$dump_query = shell_exec("ssh -i ".escapeshellarg(Conf::$extractor_config['latest_staged']['identity'])." ".escapeshellarg(Conf::$extractor_config['latest_staged']['server'])." '".$remote_command."'");
	...
	function check_list_format($dump_query_rows) {
		...
		foreach ($dump_query_rows as $row_count => $dump_query_row) {
			...
			if ($row_count % 3 === 0) {
				...
				if (preg_match(Conf::$extractor_config['latest_staged']['dump_path_check_regex'], $dump_query_row) !== 1) {
	...
	function dump_already_processed($file) {
		...
		$already_processed_query = runq("SELECT count(*) AS count FROM extract_processes p INNER JOIN extract_latest_staged f ON (f.extract_id=p.extract_id) WHERE f.source_md5='".db_escape($file['md5'])."' OR f.source_timestamp='".db_escape(date("c", $file['modtime']))."';");
	...
	function start_extract($file) {
		...
		shell_exec(Conf::$software_path."extract/extractors/latest_staged/run_extract.php ".escapeshellarg($file['path'])." ".escapeshellarg(date("c", $file['modtime']))." ".escapeshellarg($file['md5'])." > ".Conf::$software_path."logs/extractlog &");
	}
}
